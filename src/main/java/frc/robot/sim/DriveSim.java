 ...
 * <p>
 * Uses the "simple approach" ({@link SelfControlledSwerveDriveSimulation})
 * which runs
 * its own internal closed-loop on the simulated motors. Each update cycle:
 * <ol>
 * <li>Reads the desired module states from the real {@link MAXSwerveModule}
 * objects.</li>
 * <li>Feeds them into the self-controlled simulation.</li>
 * <li>Steps the physics engine forward.</li>
 * <li>Writes the resulting encoder and gyro values back into the WPILib sim
 * objects
 * so the student code's odometry "sees" realistic, slipping wheels.</li>
 * </ol>
 */
public class DriveSim {
    ...
    public DriveSim(MAXSwerveModule frontLeft, MAXSwerveModule frontRight,
        ...
        SwerveDriveSimulation swerveSim = new SwerveDriveSimulation(
                SimConstants.createDriveTrainConfig(),
                SimConstants.kStartingPose);
     ...
     * Called once per robot periodic (20 ms).
     */
    public void update() {
     ...
     *         The student code must never have access to this pose directly.
     */
