...
public class ShooterSim {
  ...
  private final SparkFlexSim m_motorSimOne;
  private final SparkFlexSim m_motorSimTwo;
  ...
  public ShooterSim(Shooter shooter) {
    ...
    m_flywheelSim =
        new FlywheelSim(
            ...
            motor);
    m_motorSimOne = new SparkFlexSim(shooter.getMotorOne(), motor);
    m_motorSimTwo = new SparkFlexSim(shooter.getMotorTwo(), motor);
  }

  /** Call every 20 ms from simulationPeriodic. */
  public void update() {
    // 1. Read voltage the motor controller is applying
    double voltage = m_shooter.getMotorOne().getAppliedOutput() * 12.0;
    ...
    m_motorSimOne.iterate(motorRPM, 12.0, 0.02);
    m_motorSimTwo.iterate(motorRPM, 12.0, 0.02);
  }
}
