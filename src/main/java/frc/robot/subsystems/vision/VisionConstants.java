 ...
 * 
 * The vision subsystem uses PhotonVision and AprilTags to estimate the robot's position on the field.
 * 
 * CAMERAS: 3x Arducam OV9281 global shutter cameras
 * - Camera 1 (front): Faces forward for speaker/amp targeting
 * - Camera 2 (front-left): Angled for wider field of view
 * - Camera 3 (back): Faces backward for defense and repositioning
 * 
 * POSE ESTIMATION PROCESS:
 * 1. Each camera detects AprilTags (fiducial markers) on the field
 * 2. PhotonVision calculates 3D pose based on tag positions and camera transforms
 * 3. Vision subsystem validates pose estimates (field bounds, jump detection, ambiguity)
 * 4. Valid poses are fused with wheel odometry in the drive subsystem
 * 5. Result: More accurate robot position than wheels alone (no drift)
 * 
 * MULTI-TAG vs SINGLE-TAG:
 * - Multi-tag (coprocessor): Uses multiple AprilTags simultaneously (most accurate)
 * - Single-tag (fallback): Uses one tag with ambiguity filtering (less reliable)
 * 
 * VALIDATION CHECKS:
 * - Field bounds: Reject poses outside the field perimeter
 * - Jump detection: Reject poses that teleport too far from current odometry
 * - Pitch/roll filtering: Ignore vision when robot is tipped (bumps, collisions)
 * - Ambiguity threshold: Reject single-tag estimates with flipped pose risk
 * 
 * TYPICAL OPERATION:
 * 1. Robot moves around field while cameras continuously scan for AprilTags
 * 2. Vision subsystem processes estimates and publishes diagnostics to Elastic
 * 3. Valid poses update drive odometry for accurate auto-aim and autonomous
 * 4. Residual telemetry shows difference between vision and wheel odometry
 */
public final class VisionConstants {
  ...
  public static final Transform3d ROBOT_TO_CAMERA_1 =
      new Transform3d(camera_y, -camera_x, camera_h, new Rotation3d(0, 0, 0));  // DEBUG:CAMERA_1_TRANSFORM
  public static final Transform3d ROBOT_TO_CAMERA_2 =
      new Transform3d(camera_y, camera_x, camera_h, new Rotation3d(0, 0, 0));  // DEBUG:CAMERA_2_TRANSFORM
  public static final Transform3d ROBOT_TO_CAMERA_3 =
      new Transform3d(camera_y - 0.0889, 0, camera_h, new Rotation3d(0, 0, Math.PI));  // DEBUG:CAMERA_3_TRANSFORM
